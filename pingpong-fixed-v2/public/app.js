...
$("btn-create-password").addEventListener("click", async () => {
  ...
  const r = await api("/api/auth/set-password", "POST", { mobile, otp, password });
  if (r.success) {
    me = r.user;
    saveSession();
    connectSocket();
...
window.addEventListener("message", (ev) => {
  ...
  if (data.type === "FOODWHEEL_BALANCE") {
    // Game balance changed (bet placed or win) — sync to the real wallet
    // almost immediately (short debounce only to collapse back-to-back
    // messages from the same tick) so winnings land in real time.
  ...
  if (data.type === "TEENPATTI_BALANCE") {
    // Table wallet changed (bet placed or win) — same fast real-time sync
    // as Food Wheel.
    if (roomTvSyncTimer) clearTimeout(roomTvSyncTimer);
    roomTvSyncTimer = setTimeout(() => {
