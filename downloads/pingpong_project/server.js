...
// Normalizes the mobile the same way every auth endpoint does (see
// normalizeMobile above) before the lookup. Bug fix: this previously did a
// raw `users[req.params.mobile]` lookup with no normalization — every other
// endpoint that keys into `users` by mobile normalizes first, so a mobile
// string that was even slightly different in shape (leading zero, stray
// whitespace, etc.) would miss here even though the account genuinely
// existed. On the client, this endpoint is called once on every page
// refresh (bootstrap()) to re-verify the session, and a miss there is
// treated as "account no longer exists" — forcing a real logout even
// though the account, and the data check itself, were both fine.
...
app.post("/api/room/create", (req, res) => {
    ...
    const room = {
        ...
        agencyId: null, roomLocked: false, gameEnabled: true, treasureChest: freshChestState(),
        mutedUntil: {}, seatLabels: {}, createdAt: new Date().toISOString()
    };
    rooms[roomId] = room;
    saveRoomsToDisk();
    const found = findUserByUserId(userId);
...
const FRUIT_WHEEL_CHIPS = [10, 100, 500, 1000, 5000, 10000];
const FW_BETTING_MS = 20000;
const FW_SPINNING_MS = 5000;
const FW_RESULT_MS = 3000;
const fruitWheelRooms = {}; // roomId -> { phase, bets, resultFoodId, phaseEndsAt, viewers, timer, emptyTimer }
...
function fwResolveSpin(roomId) {
    ...
    Object.keys(g.bets).forEach((userId) => {
        const betOnWin = g.bets[userId][g.resultFoodId] || 0;
        if (betOnWin <= 0) return;
        ...
        winners.push({ userId, name: found.user.name, amount });
    });
    if (winners.length) saveUsers();
    winners.sort((a, b) => b.amount - a.amount);
    io.to(roomId).emit("fruitwheel-winners", { resultFoodId: g.resultFoodId, winners: winners.slice(0, 3) });
    g.phase = "result";
...
function fwEnsureRoom(roomId) {
    ...
    const g = { phase: "betting", bets: {}, resultFoodId: null, phaseEndsAt: 0, viewers: new Set(), timer: null, emptyTimer: null };
...
io.on("connection", (socket) => {
    ...
    socket.on("send-gift", ({ roomId, targetUserId, targetUserIds, giftId, quantity }) => {
        const room = rooms[roomId];
        if (!room || !socket.userId) return;
        ...
        if (!uniqueIds.length) { socket.emit("room-error", { message: "কাউকে বেছে নাও" }); return; }

        const targets = uniqueIds.map((uid) => findUserByUserId(uid)).filter(Boolean);
        if (!targets.length) { socket.emit("room-error", { message: "ইউজার পাওয়া যায়নি" }); return; }

        const perTargetAmount = gift.price * qty;
        const totalCost = perTargetAmount * targets.length;
        if (senderFound.user.coins < totalCost) { socket.emit("room-error", { message: "পর্যাপ্ত কয়েন নেই" }); return; }
        ...
        targets.forEach((targetFound) => {
            // Changed on request: gift recipients now earn coins, not diamonds.
            targetFound.user.coins = (targetFound.user.coins || 0) + perTargetAmount;
            ...
            logTransaction(targetFound.user.userId, "coins", perTargetAmount, `Received ${label} from ${senderFound.user.name}`);
            ...
            io.to(roomId).emit("gift-received", { fromUserId: senderFound.user.userId, fromName: senderFound.user.name, toUserId: targetFound.user.userId, gift, quantity: qty });
        });
    ...
    socket.on("send-video-gift", ({ roomId, targetUserId, targetUserIds, videoGiftId, quantity }) => {
        const room = rooms[roomId];
        if (!room || !socket.userId) return;
        const senderFound = findUserByUserId(socket.userId);
        if (!senderFound) return;
        ...
        if ((senderFound.user.coins || 0) < totalCost) { socket.emit("room-error", { message: "পর্যাপ্ত কয়েন নেই" }); return; }
    ...
    socket.on("mod-mute-users", ({ roomId, targetUserIds, minutes }) => {
        ...
        const ids = Array.isArray(targetUserIds) ? targetUserIds.filter((uid) => uid !== room.hostId) : [];
    ...
    socket.on("mod-invite-to-seat", ({ roomId, targetUserIds }) => {
        ...
        ids.forEach((uid, idx) => {
            const sid = socketsByUserId[uid];
            const seatNumber = openSeats[idx % openSeats.length];
            if (sid) io.to(sid).emit("seat-invite", { roomId, seatNumber, fromName: hostFound ? hostFound.user.name : "Host" });
        });
    });
    ...
    socket.on("mod-move-to-audience", ({ roomId, targetUserIds }) => {
        const room = rooms[roomId];
        if (!room || !isOwnerOrAdmin(room, socket.userId)) return;
        const ids = new Set((Array.isArray(targetUserIds) ? targetUserIds : []).filter((uid) => uid !== room.hostId));
        if (!ids.size) return;
        let changed = false;
        ...
        if (changed) io.to(roomId).emit("room-state", publicRoom(room));
    });
    ...
    socket.on("mod-label-users", ({ roomId, targetUserIds, text, color }) => {
        ...
        const ids = Array.isArray(targetUserIds) ? targetUserIds : [];
        ids.forEach((uid) => {
            if (cleanText) room.seatLabels[uid] = { text: cleanText, color: (color || "#F4C463").slice(0, 20) };
    ...
    socket.on("mod-announce-users", ({ roomId, targetUserIds, message }) => {
        ...
        ids.forEach((uid) => {
            const sid = socketsByUserId[uid];
            if (sid) io.to(sid).emit("mod-announcement", { fromName: hostFound ? hostFound.user.name : "Host", message: text });
        });
    });
    ...
    socket.on("fruitwheel-bet", ({ roomId, foodId, amount }) => {
        ...
        if (!FRUIT_WHEEL_FOODS.some((f) => f.id === foodId)) return;
        amount = Math.floor(Number(amount) || 0);
        if (!FRUIT_WHEEL_CHIPS.includes(amount)) return;
        const found = findUserByUserId(socket.userId);
        if (!found || found.user.coins < amount) return;
        found.user.coins -= amount;
        saveUsers();
