...
function hydrateSeatMap(seats) {
  ...
  seats.forEach((seat, i) => {
    if (seat) {
      seatMap[i + 1] = { userId: seat.userId, socketId: seat.socketId, userName: seat.userName, userPhoto: seat.userPhoto, role: seat.role, activeFrame: seat.activeFrame || null, vipLevel: seat.vipLevel || 0, customTag: seat.customTag || null };
...
function sendVideoGift(videoGiftId) {
  ...
  if (gift && (me.coins || 0) < gift.price) { toast("পর্যাপ্ত কয়েন নেই"); return; }
  socket.emit("send-video-gift", { roomId: currentRoomId, targetUserId, videoGiftId });
  $("modal-gift").classList.add("hidden");
}
...
window.addEventListener("message", async (ev) => {
  ...
  if (data.type === "FOODWHEEL_BALANCE") {
    // Game balance changed (bet placed or win) — update the local wallet
    // immediately so switching to the other game (or anywhere else showing
    // coins) reflects it right away, then sync to the server almost
    // immediately after (short debounce only to collapse back-to-back
    // messages from the same tick) so it stays authoritative too.
...
async function bootstrap() {
  ...
  if (!r.success) {
    ...
    // The server responded and confirmed this account no longer exists
    // (e.g. an admin deleted it) — don't let a deleted account keep
