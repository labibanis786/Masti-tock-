...
app.post("/api/admin/video-gifts/upload", requireAdmin, uploadVideoGift.fields([
    ...
    if (!Number.isFinite(price) || price < MIN_VIDEO_GIFT_PRICE) {
        return res.json({ success: false, message: `Diamond Price কমপক্ষে ${MIN_VIDEO_GIFT_PRICE} হতে হবে` });
    }
...
io.on("connection", (socket) => {
    ...
    // Video Gift System — spends Diamonds (not coins) and, only once the
    // spend succeeds, broadcasts the full-screen video to everyone currently
    // in the room. Broadcasting only on success means a failed/insufficient
    // send never triggers playback for anyone.
    socket.on("send-video-gift", ({ roomId, targetUserId, videoGiftId }) => {
        ...
        if (!gift) { socket.emit("room-error", { message: "গিফট পাওয়া যায়নি" }); return; }
        if ((senderFound.user.diamonds || 0) < gift.price) { socket.emit("room-error", { message: "Insufficient Diamonds" }); return; }
        const targetFound = targetUserId ? findUserByUserId(targetUserId) : null;

        senderFound.user.diamonds -= gift.price;
        senderFound.user.vipLevel = vipLevelFromDiamonds(senderFound.user.diamonds);
        saveUsers();
        pushWalletUpdate(senderFound.user.userId);
        logTransaction(senderFound.user.userId, "diamonds", -gift.price, `Sent video gift ${gift.name}${targetFound ? ` to ${targetFound.user.name}` : ""}`);
        logGift({
