...
// price in Diamonds instead of Coins, sent through sendVideoGift().
...
// Video Gift send — spends Diamonds, requires at least 100,000. Server has
// the final say; this is just a snappy client-side pre-check.
function sendVideoGift(videoGiftId) {
  const targetUserId = $("gift-target-select").value;
  if (!targetUserId) { toast("কাউকে বেছে নাও"); return; }
  const gift = VIDEO_GIFT_CATALOG_CACHE.gifts.find(g => g.id === videoGiftId);
  if (gift && (me.diamonds || 0) < gift.price) { toast("Insufficient Diamonds"); return; }
